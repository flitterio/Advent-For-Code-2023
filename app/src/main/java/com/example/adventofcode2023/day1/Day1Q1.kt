...
object Day1Q1 {
    ...
    fun run() {
        val digitNumberList = getPart1Numbers(values)
//        println("Question1: number list" + digitNumberList)
//        println("Question1: sum ${digitNumberList.sum()}")
        println("question2: ${getPart2Numbers(values).sum()}")
    }

    fun getPart1Numbers(list: List<String>): ArrayList<Int> {
        val numbers = arrayListOf<Int>()

        list.forEach { value ->
            numbers.add(getNum(value).toInt())
        }
        return numbers
    }

    fun getPart2Numbers(list: List<String>): ArrayList<Int> {
        val listOfNumbers = arrayListOf<Int>()
        list.forEach {value ->
            var stringWithNumber = value
            stringWithNumber = stringWithNumber.replace("one", "o1e", ignoreCase = true)
            stringWithNumber = stringWithNumber.replace(oldValue = "two", newValue = "t2o", ignoreCase = true)
            stringWithNumber = stringWithNumber.replace(oldValue = "three", newValue = "t3e", ignoreCase = true)
            stringWithNumber = stringWithNumber.replace(oldValue = "four", newValue = "f4r", ignoreCase = true)
            stringWithNumber = stringWithNumber.replace(oldValue = "five", newValue = "f5e", ignoreCase = true)
            stringWithNumber = stringWithNumber.replace(oldValue = "six", newValue = "s6x", ignoreCase = true)
            stringWithNumber = stringWithNumber.replace(oldValue = "seven", newValue = "s7n", ignoreCase = true)
            stringWithNumber = stringWithNumber.replace(oldValue = "eight", newValue = "e8t", ignoreCase = true)
            stringWithNumber = stringWithNumber.replace(oldValue = "nine", newValue = "n9e", ignoreCase = true)

            println("value $value stringwithnumber $stringWithNumber num ${getNum(stringWithNumber)}")
            listOfNumbers.add(getNum(stringWithNumber).toInt())
        }
        println("question2: ${listOfNumbers}")
        return listOfNumbers
    }

    fun getNum(value: String): String {
        var num = "0"
        for (c in value) {
            if (c.isDigit()) {
                num = c.toString()
